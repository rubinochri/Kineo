...
app.get('/api/videos', async (req, res) => {
  try {
    const listaVideo = await Video.find(); // Cerca TUTTI i documenti nella collezione Video
    res.json(listaVideo); // Invia l'array di video al frontend in formato JSON
  } catch (errore) {
    res.status(500).json({ message: errore.message }); // Se errore server, invia status 500 e messaggio
  }
});
...
app.get('/api/videos/:id', async (req, res) => { //:id indica una variabile
  try {
    const videoTrovato = await Video.findById(req.params.id); // Cerca il video usando l'ID passato nell'URL
    if (!videoTrovato) return res.status(404).json({ message: 'Video non trovato' }); // Se non esiste, 404
    res.json(videoTrovato); // Restituisce il video trovato
  } catch (errore) {
    res.status(500).json({ message: errore.message }); // Errore generico
  }
});
...
app.post('/api/videos', async (req, res) => {
  try {
    ...
    res.status(400).json({ message: errore.message }); // Errore client (dati sbagliati)
  }
});
...
app.patch('/api/videos/:id', async (req, res) => {
  try {
    const datiAggiornamento = req.body; // Prende i campi da aggiornare
    const opzioniQuery = { new: true, runValidators: true }; // new: true restituisce il doc aggiornato, non il vecchio
    ...
    res.json(videoModificato); // Restituisce il video con le modifiche applicate
  } catch (errore) {
...
app.delete('/api/videos/:id', async (req, res) => {
  try {
    const videoEliminato = await Video.findByIdAndDelete(req.params.id); // Cerca ed elimina in un colpo solo
