...
export default function DashboardAmministratore() {
  ...
  useEffect(() => {
    ...
    if (!utenteStringa) {
        // Ritorna alla pagina di login del portale principale
    ...
    try {
      ...
      // Rendi il controllo case-insensitive e supporta sia 'ruolo' che 'role'
      const ruolo = (utente.ruolo || utente.role || '').toLowerCase();
      if (ruolo !== 'admin') {
      ...
      console.error("Errore durante il parsing di userData:", e);
      window.location.href = '/login';
    }
  ...
  const gestisciLogout = () => {
    ...
    // 2. Manda alla homepage del portale
